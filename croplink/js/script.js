...
//fiterable menu
//=============================
  // Get the list items and product images
const liItem = document.querySelectorAll('#products ul li');
const imgItem = document.querySelectorAll('.featured-product-wrap .product-wrap');
...
liItem.forEach(li => {
  li.onclick = function() {
    ...
    // Filter
    const selectedCategory = li.textContent.toLowerCase();
    imgItem.forEach(item => {
      ...
      switch (selectedCategory) {
        case 'all':
          ...
          break;
        case 'grains':
          // Show only products in the "grains" category
          ...
          break;
        case 'meat':
          // Show only products in the "meat" category
          ...
          break;
        case 'fruits':
          // Show only products in the "fruits" category
          ...
          break;
        case 'dairy':
          // Show only products in the "dairy" category
          ...
          break;
        default:
          // Hide products if no matching category is found
          item.style.display = 'none';
          break;
      }
    });
...
function createShoppingCartItem(itemName, itemPrice, itemUnit, itemDiscount, presentPrice, itemQuantity) {
    ...
    newChildPara[2].innerHTML =  `$` + itemPrice + ' /kg'  ;
    newChildPara[3].innerHTML = itemDiscount + `%`;
    newChildPara[4].innerHTML = `$` + presentPrice + ' /kg' ;
...
function addItemsToSelectedProducts(productIndex) {
  ...
  itemsToBuy.push({
    name: productCartName,
    price: productCartPrice
  });
}
$(document).ready(function() {
  $('#confirm-order-btn').on('click', function() {
    var cartItems = getCartItems();
    var cartItemsInput = $('#cartItemsInput');
  ...
  function getCartItems() {
    // Retrieve and format the shopping cart items
    var cartItems = [];
    var cartItemElements = $('.calculate-buying-details');

    cartItemElements.each(function(index, itemElement) {
      var titleElement = $(itemElement).find('.product-name');
      var priceElement = $(itemElement).find('.f-product-price');

      var cartItem = {
        titleInput: titleElement.text(),
        priceInput: priceElement.text()
      };
